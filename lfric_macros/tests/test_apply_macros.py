...
def test_find_meta_dirs():
    result = applymacros.find_meta_dirs(TEST_APPS_DIR / "rose-meta")
    expected = set()
    expected.add(TEST_META_DIR / "lfric-lfric_atm")
    expected.add(TEST_META_DIR / "lfric-gungho")
    expected.add(TEST_META_DIR / "lfric-driver")
    expected.add(TEST_META_DIR / "um-iau")
    expected.add(TEST_META_DIR / "lfric-transport")
...
def test_read_python_imports():
    ...
    test_file = TEST_APPS_DIR / "test_read_python_imports.py"
    expected = set()
    imports = (
        (("shlex",), ("split",), None),
...
def test_find_macro():
    ...
    with pytest.raises(Exception, match=expected_error):
        applymacros.find_macro(Path("meta_dir"), [""])
...
def test_parse_macro():
    for macro in (existing_macro, desired_macro):
        applymacros.parsed_macros["meta_dir"].append(
            applymacros.parse_macro(macro, Path("meta_dir"))
        )
    ...
    with pytest.raises(Exception, match=r".*failed/versions.py"):
        applymacros.parse_macro("", Path("failed"))


def test_check_missing_macro():
    macros = applymacros.parsed_macros["meta_dir"]
    applymacros.parsed_macros[TEST_META_DIR / "lfric-gungho"] = macros
    missing = applymacros.check_missing_macros(
        TEST_META_DIR / "lfric-lfric_atm", ["lfric-gungho"]
    )
    assert missing == ["vn0.0_t000"]
    missing = applymacros.check_missing_macros(TEST_META_DIR / "lfric-gungho", [])
    assert missing == []


def test_combine_missing_macros():
    combined = applymacros.combine_missing_macros(
        [TEST_META_DIR / "lfric-gungho"], ["vn0.0_t000"]
    )
    expected_combined = {
        "vn0.0_t000": {
            "before_tag": "vn0.0",
...
def test_read_meta_imports():
    ...
    applymacros.parsed_macros[TEST_APPS_DIR]["imports"] = applymacros.read_meta_imports(
        TEST_APPS_DIR / "rose-meta" / "lfric-gungho"
    )
    expected_imports = [
        applymacros.root_path / "rose-meta" / "lfric-driver",
        applymacros.root_path / "rose-meta" / "um-iau",
    ]
...
def test_order_meta_dirs():
    applymacros.target_macros = {
        TEST_META_DIR / "lfric-driver": {"imports": []},
        TEST_META_DIR / "lfric-gungho": {
            "imports": [
                TEST_META_DIR / "lfric-driver",
                TEST_META_DIR / "um-iau",
            ]
        },
        TEST_META_DIR / "um-iau": {"imports": []},
        TEST_META_DIR / "lfric-lfric_atm": {
            "imports": [TEST_META_DIR / "lfric-gungho"]
        },
        TEST_META_DIR / "lfric-transport": {
            "imports": [TEST_META_DIR / "lfric-gungho"]
        },
    }
    order = applymacros.order_meta_dirs()
    gungho = order.index(TEST_META_DIR / "lfric-gungho")
    lfric_atm = order.index(TEST_META_DIR / "lfric-lfric_atm")
    driver = order.index(TEST_META_DIR / "lfric-driver")
    um = order.index(TEST_META_DIR / "um-iau")
...
def test_get_rose_apps():
    expected = set()
    expected.add(TEST_ROSE_STEM / "gungho")
    expected.add(TEST_ROSE_STEM / "lfric_atm")
    expected.add(TEST_ROSE_STEM / "transport")
...
def test_apps_to_upgrade():
    ...
    expected = (
        [TEST_ROSE_STEM / "gungho", TEST_ROSE_STEM / "lfric_atm"],
        [TEST_ROSE_STEM / "lfric_atm", TEST_ROSE_STEM / "gungho"],
    )
